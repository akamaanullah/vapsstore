document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderTable(combinations) {
        ...
        Object.keys(groups).forEach((parentValue, gIndex) => {
            ...
            if (subVariants[0].length > 1) {
                html += `<tr class="variant-parent-row" data-group-target="${groupId}" style="cursor: pointer;"><td><div class="fs-14 fw-500">${parentValue}</div><div class="text-muted-sm">${subVariants.length} variants <i data-lucide="chevron-down" class="icon-xxs chevron-toggle"></i></div></td><td>$${basePrice}</td><td>Multiple</td></tr>`;
    ...
    if (tagInput) {
        ...
        // Clear text field but keep badges
        const initialValue = tagInput.value;
        if (initialValue && selectedTagsArr.length > 0) tagInput.value = '';
    }
