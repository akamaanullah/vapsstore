document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addVariantBtn && variantsContainer) {
        ...
        function addOptionRow() {
            ...
            row.innerHTML = `
                    ...
                    <div class="variant-option-inner">
                        <div class="variant-content-area">
                            ...
                            <div class="variant-card-footer mt-20">
                                <button class="btn-link text-error fs-14 remove-option-btn">Delete</button>
                                <button class="btn btn-dark btn-sm done-option-btn">Done</button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="option-summary-view d-none" style="position: relative; width: 100%; min-height: 45px; display: flex; align-items: center; padding: 0 15px; background: #fff; border: 1px solid var(--border-color); border-radius: 8px;">
                    ...
                    <button class="btn-action-icon edit-option-btn" title="Edit" style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%); display: flex; align-items: center; justify-content: center;">
                ...
                if (subVariants[0].length > 1) {
                    ...
                    subVariants.forEach(combo => {
                        ...
                        html += `
                            <tr class="variant-child-row ${groupId}" style="display: none;">
                                ...
                                <td><span class="fs-13">${subName}</span></td>
                                <td>
                                    <div class="input-prefix-container">
                                        <span class="prefix">$</span>
                                        <input type="number" class="modal-field-input input-sm variant-price-input" value="0.00" step="0.01">
                                    </div>
                                </td>
                                <td>
                                    <input type="number" class="modal-field-input input-sm variant-qty-input" value="0">
                                </td>
                            </tr>
                        `;
                    });
                    ...
                    html += `
                        <tr>
                            ...
                            <td><span class="fs-14">${parentValue}</span></td>
                            <td>
                                <div class="input-prefix-container">
                                    <span class="prefix">$</span>
                                    <input type="number" class="modal-field-input input-sm variant-price-input" value="0.00" step="0.01">
        ...
        function addCollection(name) {
            if (selectedCollectionsArr.includes(name)) return;
            selectedCollectionsArr.push(name);
            renderCollections();
        }

        function removeCollection(name) {
            const index = selectedCollectionsArr.indexOf(name);
            if (index > -1) {
                selectedCollectionsArr.splice(index, 1);
                renderCollections();
            }
        }

        function renderCollections() {
            colSelectedDiv.innerHTML = selectedCollectionsArr.map(c => `
                <div class="tag-badge">
                    ${c}
                    <span class="remove-tag" onclick="window.removeCol('${c}')">
        ...
        window.removeCol = removeCollection;
        ...
        function removeTag(name) {
            const index = selectedTagsArr.indexOf(name);
            if (index > -1) {
        ...
        function renderTags() {
            tagSelectedDiv.innerHTML = selectedTagsArr.map(t => `
                <div class="tag-badge">
                    ${t}
                    <span class="remove-tag" onclick="window.removeTagItem('${t}')">
                        <i data-lucide="x" class="icon-xxs"></i>
                    </span>
                </div>
            `).join('');
        ...
        window.removeTagItem = removeTag;
